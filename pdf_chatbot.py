...
def create_chain(document_store):
    ...
    #create conversation system with source document retrieval
...
#image for bot
bot_template = "<img src='https://upload.wikimedia.org/wikipedia/en/5/5b/WALL%C2%B7E_%28character%29.jpg' style='max-height: 78px; max-width: 78px; border-radius: 50%; object-fit: cover;'> &nbsp; &nbsp;"

#image for user
user_template = " &nbsp;<img src='https://static.tvtropes.org/pmwiki/pub/images/linguinei_feom_ratatsaoiulle_9937.png' style='max-height: 78px; max-width: 78px; border-radius: 50%; object-fit: cover;'> &nbsp; &nbsp; &nbsp;"
...
def process_question(input_query):
    ...
    for question_num, message in enumerate(st.session_state.chat_history):
        user = []
        assistant = []
        #append answers to list so that user is above assistant with the most recent text
        if question_num % 2 == 0:
            user.append(message)
            st.write(f"{user_template}{message.content}", unsafe_allow_html=True)
        else:
            assistant.append(message)
            st.write(f"{bot_template}{message.content}", unsafe_allow_html=True)
